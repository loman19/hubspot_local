...
const WORKFLOW_NAME = "03B_REG_BATCH";
...
class WorkflowAction {
    ...
    async execute() {
        ...
        try {
            ...
            // OLD: Get registration IDs of all skipped registrations
            // const allSkippedIds = [
            ...
            // OLD: Add skipped registration IDs directly to failed array
            ...
            // NEW: Add skipped ACCOUNT IDs directly to failed array
            if (allSkippedAccountIds.length > 0) {
                console.log(`[SKIP] ${allSkippedAccountIds.length} account IDs marked as failed (no account ID or invalid email)`);
            ...
            // OLD: Add registration IDs directly to failed array
            // this.queue.data.items.failed.push(...registrationIds.map(id => String(id)));

            // NEW: Add ACCOUNT IDs directly to failed array (extract from registrations if available)
            ...
            for (const registration of registrations) {
                ...
                if (email) {
                    ...
                    if (!isValidEmail) {
                        ...
                        if (this.errorManager) {
                            // OLD: Did not include accountId or registrationId
                            // this.errorManager.logError(new Error(validationResult.reason || 'Invalid email'), {
                            //     email: email,
                            //     name: getDisplayName(registration) || 'Unknown'
                            // }, {
                            //     field: 'email'
                            // });
                ...
                for (let i = 0; i < contactsToCreate.length; i += MAX_BATCH_SIZE) {
                    ...
                    try {
                        ...
                        if (error.response?.body?.errors) {
                            ...
                            // Batch API error - log once at batch level, not per-contact
                            console.error(`[ERROR] Batch create failed: ${error.message}`);
                            if (this.errorManager) {
                                this.errorManager.logError(error, {
            ...
            if (contactsToUpdate.length > 0) {
                ...
                for (let i = 0; i < contactsToUpdate.length; i += MAX_BATCH_SIZE) {
                    ...
                    try {
                        ...
                        // Batch API error - log once at batch level, not per-contact
                        console.error(`[ERROR] Batch update failed: ${error.message}`);
                        if (this.errorManager) {
                            this.errorManager.logError(error, {
                                batchSize: batch.length
                            }, {
...
class ProcessRegistrationBatchAction extends EventActionBase {
    async execute() {
        ...
        try {
            ...
            // Log missing account_id errors - don't add to failed queue
            if (skippedRegistrations.length > 0 && this.errorManager) {
                skippedRegistrations.forEach(reg => {
                    // OLD: Did not include accountId or registrationId
                    // this.errorManager.logError(new Error('Missing required field: account_id'), {
                    //     email: reg.email,
                    //     name: getDisplayName(reg) || 'Unknown'
                    // }, {
                    //     field: 'account_id'
                    // });
            ...
            // Get IDs of all skipped registrations (for logging only, not queue tracking)
            const allSkippedIds = [...skippedRegistrationIdsFromMissing, ...skippedRegistrationIds];

            // Update queue - only mark successful ones as processed
            // Failed contacts are logged to error logs, not tracked in queue
            if (successfulIds.length > 0) {
                this.queue.markProcessed(successfulIds);
            }

            // Don't add failed contacts to queue - they're logged in error logs
            if (allSkippedIds.length > 0) {
                console.log(`[SKIP] ${allSkippedIds.length} registrations skipped (logged to error logs)`);
            }

            this.queue.updateRemaining();
            console.log(`[SUMMARY] Total: ${registrationIds.length}, Success: ${successfulIds.length}, Skipped: ${allSkippedIds.length}`);

        } catch (error) {
            // Batch processing error - log to error logs, don't add to failed queue
            console.error(`[ERROR] Batch processing failed: ${error.message}`);
            if (this.errorManager) {
                this.errorManager.logError(error, {
            ...
            // Don't add to failed queue - error is logged
            this.queue.updateRemaining();
        }
